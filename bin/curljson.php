...
// apt-get install php7.0-cli php7.0-curl
...
function json_to_array(array $array, $pathSeparator = '.')
{
    $result = [];
    ...
    while ($array)
    {
        ...
        if (is_array($value))
        {
            ...
            {
                $build[$key . $pathSeparator . $subKey] = $node;
            }
            $array = $build + $array;
...
else if (isset($opts["feature"]))
{
    ...
    switch($opts["feature"])
    {
                ...
                foreach ($json as $jsonkey => $jsonvalue)
                {
                    ...
                    foreach (array_values($opts["macro"]) as $m)
                    {
                        ...
                        else
                        {
                            ...
                            if ($opts["feature"] == "zabbix-lld-urlencode")
                            {
                              $name = sprintf("{#%sURL}", strtoupper(get_lastvalue($m)));
                              $item[$name] = urlencode(array_value($jsonvalue, $m));
                            }
                        }
                    }
                    $discovery[] = $item;
        ...
        case "zabbix-sender":
            ...
            $json_array = json_to_array($json);
            foreach (array_keys($json_array) as $item)
            {
                echo '"<hostname>" "' . $item . '" "' . $json_array[$item] . "\"\n";
            }
            break;
        default:
